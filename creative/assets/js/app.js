...
function makeTrackCard(track, index, featured = false) {
  ...
  img.src = track.cover;
  img.alt = `Обложка трека «${track.title}»`;
  img.loading = featured ? 'eager' : 'lazy';
  img.decoding = 'async';
  img.addEventListener('error', () => { img.src = 'assets/covers/default-cover.jpg'; });
  ...
  fav.setAttribute('aria-label', state.favorites.has(track.id) ? 'Убрать из избранного' : 'Добавить в избранное');
  top.append(meta, fav);
  ...
  desc.textContent = track.description || 'Музыкальная работа KnowerLife.';
  ...
  actions.append(
    button('Слушать', 'btn btn--primary btn--small', () => playTrack(index)),
    button('Подробнее', 'btn btn--secondary btn--small', () => openTrackDialog(index))
  );
...
function renderGenres() {
  ...
  const all = button('Все', 'filter-chip', () => setGenre('all'));
...
function updateStats() {
  ...
  els.totalDuration.textContent = seconds ? `${Math.max(1, Math.round(seconds / 60))} мин` : '—';
}
...
function loadTrack(index, autoplay = false) {
  ...
  els.playerCover.alt = `Обложка «${track.title}»`;
...
async function playCurrent() {
  ...
  try {
    ...
    toast('Браузер не разрешил автоматическое воспроизведение.');
  }
}
...
async function shareTrack() {
  ...
  const data = { title: `${track.title} — ${track.artist}`, text: track.description || 'Слушайте в KnowerLife Creative Corner', url };
  if (navigator.share) {
  ...
  try {
    ...
    toast('Скопируйте адрес страницы из браузера');
  }
}
...
function openTrackDialog(index) {
  ...
  els.dialogCover.alt = `Обложка «${track.title}»`;
  ...
  els.dialogStory.textContent = track.story || 'История создания пока не добавлена.';
  els.dialogLyrics.textContent = track.lyrics || 'Текст пока не добавлен.';
...
function onAudioPlay() {
  ...
  els.playerPlay.setAttribute('aria-label', 'Пауза');
...
function onAudioPause() {
  els.playerPlay.textContent = '▶';
  els.playerPlay.setAttribute('aria-label', 'Воспроизвести');
...
function setupAccessibility() {
  ...
  const apply = () => {
    ...
    if (els.accessibilityToggle) { els.accessibilityToggle.setAttribute('aria-pressed', String(enabled)); els.accessibilityToggle.textContent = enabled ? 'A' : 'A+'; const label = enabled ? 'Обычный режим отображения' : 'Версия для слабовидящих'; els.accessibilityToggle.setAttribute('aria-label', label); els.accessibilityToggle.title = label; }
  };
...
function setupControls() {
  ...
  els.audio.addEventListener('error', () => {
    toast('Не удалось загрузить аудиофайл. Проверьте путь в tracks.json.');
    onAudioPause();
  });
...
async function init() {
  ...
  try {
    ...
    state.tracks = [];
    toast('Каталог музыки пока недоступен.');
  }
